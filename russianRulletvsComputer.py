...
player = [{          # 플레이어 리스트 {내부 딕셔너리} 선언
    ...
    'dmg':1,         # dmg 데미지
    'item':[],       # item 아이템리스트
    'stat':0         # stat 상태 아이템용 0 : 정상, 1 : 오함마 대기중, 2 : 최면상태
    },
    {
    'name':'dealer',
    'hp':4,
...
turn = 0     # 턴 계산용 변수 ==> 0: 1플레이어, 1: 2플레이어
...
# --------  벅샷 조준, 사격 메서드 -------------------------------------------------------------------------
def aimingBugshot(guunner,target):                  # target(사격자, 타겟)
    print('끼릭 ...... 철컥 !')
    print(playMent[guunner][abs(guunner - target)])
    input()
    if player[guunner]['stat'] == 2:                # 사격자 상태 체크
        print(f'총구가 반대로 이동합니다.{player[guunner]["name"]}을 조준합니다.')
        player[guunner]['stat'] = 0                 # 플레이어 스텟 클리어
        input()
    if bugshot[0]:                                  # bugshot 리스트 첫번째 값이 1 : 트루일경우
        shotBugshot(guunner,target)                 # 격발 메서드 실행
        return True                                 # 턴 넘김 유무를 위해 값 반환
    else :
        print('틱........................ 격발되지 않았습니다.')
        if player[guunner]['dmg'] != 1 : player[guunner]['dmg'] = 1
        del bugshot[0]
        return False 

# --------  벅샷 격발 유무 메서드 -------------------------------------------------------------------------
def shotBugshot(guunner,target):
    if not player[1]['brain'] : player[1]['brain'] == 0 # 브레인이 0이 아닐경우 0으로
    player[target]['hp'] -= player[guunner]['dmg']      # 선언된 상대에게 데미지 만큼 마이너스
    if player[guunner]['dmg'] == 1 :
        print(f'탕! - 격발되었습니다. {player[target]["name"]}의 체력이 감소합니다.')
    else:
        print(f'타 - 앙!!! 격발되었습니다. {player[target]["name"]}의 체력이 크게 감소합니다.')
        player[guunner]['dmg'] = 1
...
def upgradeGun(target):
    ...
    player[target]['dmg'] = 2               # ..... 대상의 dmg 에 1 더하기

# -------- 아이템 최면 사용 메서드 ---------------------------------------------------------------------------
def hypnosiss(target):
    player[1-target]['stat'] = 2
    if not target : print('상대는 최면에 걸렸다.')  # ..... 대상의 stat 2 : 최면상태로 변환
...
def computerPlay():
    ...
    if player[1]['brain'] > 0 : 
        ...
        if len(player[1]['item']) > 3 or 1 in player[1]['item'] : # 아이템의 개수가 3개 이상이거나 담배가 있다면
            return 3
        if len(bugshot) - bugshot.count(1) < 3:                   # 총 탄창 - 실탄 이 3개 보다 적다면
            choice = 1
        elif bugshot.count(1) > len(bugshot) // 2:                # 총 탄찬 / 2보다 실탄이 많다면
            if len(player[1]['item']) and itemRandomStat : # 이이템에 맥주, 실탄확인이 있다면
                return 3
            choice = random.randint(1,3)%2                        # 랜덤인자 사격에 보다 집중
        else:
            choice = random.randint(1,3)%2
    return choice

def computerItemUse():                                     
    itemIdx = 0 # 선택할 아이템 기본값 0
    if( player[1]['brain'] == 0 and ( 4 in player[1]['item'] or 5 in player[1]['item'])): # 브레인이 0이고 최면과 오함마 있을경우
        player[1]['isView'] = False             # ... isview false로 변환
        return player[1]['item'].index(4) if (4 in player[1]['item']) else player[1]['item'].index(5) # 최면과 오함마가 있는 리스트 인덱스값 반환
    else :
    ...
    elif 1 in player[1]['item'] :           # 우선도 담배 > 약실확인 > 맥주 > 총기강화
        itemIdx = 1
    elif 2 in player[1]['item'] :
        itemIdx = 2
    elif 0 in player[1]['item'] :
        itemIdx = 0
    elif 3 in player[1]['item'] :
        itemIdx = 3
    return player[1]['item'].index(itemIdx) # 값 반환
...
while True :
    ...
    if turn :                       # 컴퓨터인지 검사
        ...
        playnum = int(playnum)
        print('-'*30)
    if player[turn]['stat'] == 2 and playnum < 3:       # 환각 상태고 사격을 선택했을 경우
        if turn : print('"대체 언제 건거야 ?"')
    ...
    if playnum == 1:    # 1일경우 상대 플레이어를 대상으로 aimingBugshot 실행
        ...
        if turn :       # 컴퓨터인 경우
            computerItemIdx = computerItemUse()+1        # 아이템 유스 함수 실행 반환값이 1추가
